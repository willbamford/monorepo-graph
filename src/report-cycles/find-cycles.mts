...
const STATEMENT_POLARITY = Object.freeze({
  UNKNOWN: 0,
  SAME: 1,
  OPPOSITE: -1,
});

const SELECTED_COLOR = "#56b3e9";

interface EdgeParameter {
  id: string;
  source: string;
  target: string;
  model_id?: string;
...
class Graph {
  ...
  constructor(edges: Edge[]) {
    ...
    edges.forEach((e) => {
      const sourceVertex = vertices[e.source]
        ? vertices[e.source]
        : new Vertex(e.source);
      const targetVertex = vertices[e.target]
        ? vertices[e.target]
        : new Vertex(e.target);
  ...
  findCycles() {
    ...
    while (sccs.length > 0) {
      ...
      const stack = [
        [startNode, Array.from(this.adjacentVertices[startNode.name])],
      ];
      while (stack.length > 0) {
        ...
        if (neighbors.length > 0) {
          ...
          if (nextNode === startNode) {
            ...
            stack.push([
              nextNode,
              Array.from(this.adjacentVertices[nextNode.name]),
            ]);
...
export function hasBackingEvidence(edge: EdgeParameter) {
  ...
  else if (edge.polarity === STATEMENT_POLARITY.OPPOSITE)
